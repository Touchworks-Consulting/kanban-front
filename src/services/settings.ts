...
class SettingsService {
  ...
  async updateProfile(
    ...
    const response = await api.put<ApiResponse<{ profile: ProfileData }>>(
  ...
  async exportLeads(options: ExportOptions = {}): Promise<Blob> {
    ...
    const response = await api
      .getAxiosInstance()
      .get(`/api/settings/export/leads?${params.toString()}`, {
        responseType: "blob",
      });
  ...
  async exportCampaigns(): Promise<Blob> {
    const response = await api
      .getAxiosInstance()
      .get("/api/settings/export/campaigns", {
        responseType: "blob",
      });
  ...
  async exportWebhookLogs(
    options: Pick<ExportOptions, "dateFrom" | "dateTo"> = {}
  ): Promise<Blob> {
    const params = new URLSearchParams();
    ...
    const response = await api
      .getAxiosInstance()
      .get(`/api/settings/export/webhook-logs?${params.toString()}`, {
        responseType: "blob",
      });
  ...
  async updateNotificationSettings(
    ...
    >("/api/settings/notifications/settings", settings);
    return response.data;
  }
  ...
  async dismissNotification(
    ...
    const response = await api.put<ApiResponse<{ message: string }>>(
      `/api/settings/notifications/${id}/dismiss`
    );
    return response.data;
  }
  ...
  async cleanupOldLogs(
    ...
    >("/api/settings/maintenance/cleanup-logs", options);
    return response.data;
  }

  // Custom Status Management
  async getCustomStatuses(): Promise<{ statuses: CustomStatus[] }> {
    const response = await api.get<{ statuses: CustomStatus[] }>(
  ...
  async getLossReasons(): Promise<{ lossReasons: LossReason[] }> {
    const response = await api.get<{ lossReasons: LossReason[] }>(
      "/api/settings/loss-reasons"
    );
    return response.data;
  }
